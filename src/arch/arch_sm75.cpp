...
namespace dada {
  bool SM75::isVariableLatency(Instruction const* instr){
    return variable_latency.find(instr->opcode_) != variable_latency.end(); 
  }

  int SM75::getLatency(Instruction const* instr){
  ...
  int SM75::getTputLatency(Instruction const* instr1, Instruction const* instr2) {
  ...
  int SM75::getParameterBaseOffset() const {
    return parameter_base_offset;
  }

  vector<uint64_t> SM75::getInstructionBinary(Instruction const* instr, RegisterAllocator const* register_allocator, CtrlLogic const& ctrl_logic) const{
    ...
    switch(opcode){
      ...
      case S2R: case CS2R:
        try{
          ...
          cout << "Error when gen operand encoding for hadd2...mov...(SM75)\n"
               << e.what() << endl;
        }
        break;
      case MOV: 
...
//--------------------------Helper-functions-------------------------
  uint64_t SM75::getOpEncoding(Instruction const* instr) const {
  ...
  uint64_t SM75::getFlagsEncoding(Instruction const* instr) const {
    uint64_t result = 0;
    for(auto const& flag : instr->flags_){
  ...
  void SM75::modifyICR(Opcode opcode, uint64_t& first, Operand const* src, int loc) const {
